...
if( !class_exists( 'Newsmatic_Theme_Info' ) ) :
    class Newsmatic_Theme_Info {
        ...
        protected $newsmatic_disable_admin_notices;
        ...
        function __construct() {
            ...
            add_action( 'wp_ajax_newsmatic_dismiss_welcome_notice', array( $this, 'newsmatic_dismiss_welcome_notice' ) );
        ...
        function theme_info_scripts($hook) {
            if( ! $this->newsmatic_disable_admin_notices ) :
                ...
                if( ! get_transient( 'newsmatic_show_review_notice' ) ) :
                    wp_enqueue_style( 'newsmatic-admin-notice', get_template_directory_uri() . '/inc/admin/assets/admin-notice.css', array(), esc_attr( $this->version ), 'all' );
                endif;
                    wp_enqueue_script( 'newsmatic-admin-notice', get_template_directory_uri() . '/inc/admin/assets/admin-notice.js', array( 'jquery' ), esc_attr( $this->version ), true );
                    wp_localize_script( 'newsmatic-admin-notice', 'newsmaticThemeInfoObject', array(
                        'ajaxUrl'   => esc_url( admin_url( 'admin-ajax.php' ) ),
                        '_wpnonce'  => wp_create_nonce( 'newsmatic-theme-info-nonce' ),
                        'dismissingText'    => esc_html__( 'Dismissing', 'newsmatic' )
                    ));
            endif;
                        ...
                        <div class="demo-importer-actions">
                            <?php
                                ...
                                switch( $this->importer_status ) {
                                    case 'inactive' : printf( esc_html__( 'Activate Blazethemes Demo Importer Now and Import any available demo in One Click', 'newsmatic') . ' %s', '<button class="newsmatic-importer-action-trigger" data-action="activate" data-process="' .esc_html__( "Activating plugin", "newsmatic" ). '">' .esc_html__( 'Activate Plugin', 'newsmatic' ). '</button>' ); 
                                                    break;
                                    case 'install'  : printf( esc_html__( 'Install BlazeThemes Demo Importer and Import any available demo in One Click', 'newsmatic') . ' %s', '<button class="newsmatic-importer-action-trigger" data-action="install" data-process="' .esc_html__( "Installing plugin", "newsmatic" ). '">' .esc_html__( 'Install and Activate Plugin', 'newsmatic' ). '</button>' );
                                                    break;
         ...
         * 
         */
        function newsmatic_dismiss_welcome_notice() {
            check_ajax_referer( 'newsmatic-theme-info-nonce', '_wpnonce' );
            if ( ! current_user_can( 'manage_options' ) ) { // Check if the current user can manage options (an administrator)
                wp_die( $this->restriction_message );
            }
            ...
            $this->ajax_response['status'] = true;
            $this->ajax_response['message'] = esc_html__( 'Welcom notice hidden', 'newsmatic' );
            $this->send_ajax_response();
        }
        ...
        ?>
            <div class="newsmatic-welcome-notice notice notice-info is-dismissible">
                <div class="notice-content-wrap">
                    <h2 class="notice-heading"><?php esc_html_e( 'Thank you for using Newsmatic theme', 'newsmatic' ); ?></h2>
                    <div class="notice-content">
                        <div class="notice-content-inner"><?php esc_html_e( 'Get started with multipurpose news theme and give your site a new look. We recommend you to please go through the documentation to get started with theme and setup homepage quicky.', 'newsmatic' ); ?></div>
                ...
                <div class="notice-dismiss-button"><?php esc_html_e( 'Dismiss this notice', 'newsmatic' ); ?></div>
            </div>
            <?php
        }
        ...
        function add_review_admin_notice() {
            ...
            ?>
            <div class="newsmatic-admin-notice notice notice-success is-dismissible">
        ...
        public function add_dashboard_widgets() {
            wp_add_dashboard_widget( 'newsmatic_blazethemes_author_overview', 
                esc_html__( 'BlazeThemes Author Overview', 'newsmatic' ),
            ...
            if ( ( !is_wp_error($blaze_blogs)) && (200 === wp_remote_retrieve_response_code( $blaze_blogs ) ) ) {
                ...
                if( json_last_error() === JSON_ERROR_NONE ) {
                    if( is_array($responseBody) ) :
                        echo '<ul class="newsmatic-blog-list">';
                        ...
                        ?>
                            <p class="community-events-footer">
                                <a href="<?php echo esc_url( 'https://blazethemes.com/blog/' ); ?>" target="_blank"><?php echo esc_html__( 'Blogs', 'newsmatic' ); ?><span class="screen-reader-text"><?php echo esc_html__( '(opens in a new tab)', 'newsmatic' ); ?></span><span aria-hidden="true" class="dashicons dashicons-external"></span></a> | 
                                <a href="<?php echo esc_url( 'https://blazethemes.com/support/' ); ?>" target="_blank"><?php echo esc_html__( 'Help', 'newsmatic' ); ?><span class="screen-reader-text"><?php echo esc_html__( '(opens in a new tab)', 'newsmatic' ); ?></span><span aria-hidden="true" class="dashicons dashicons-external"></span></a> | 
                                <a href="<?php echo esc_url( 'https://blazethemes.com/theme/newsmatic-pro/' ); ?>" target="_blank"><?php echo esc_html__( 'Upgrade', 'newsmatic' ); ?><span class="screen-reader-text"><?php echo esc_html__( '(opens in a new tab)', 'newsmatic' ); ?></span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>
                            </p>
                        <?php
                    endif;
                }
